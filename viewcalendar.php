			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<b>Schedule Booking of Each Month</b>
					</div>
					<div class="card-body">
						<div class="row">
							<label for="" class="control-label col-md-2 offset-md-2">Type Location:</label>
							<div class=" col-md-4">
							<select name="faculty_id" id="faculty_id" class="custom-select select2">
								<option value=""></option>
							<?php 
								$faculty = $conn->query("SELECT *,concat(faculty_id) as name FROM faculty order by concat(faculty_id) asc");
								while($row= $faculty->fetch_array()):
							?>
								<option value="<?php echo $row['idFC'] ?>"><?php echo ucwords($row['name']) ?></option>
							<?php endwhile; ?>
							</select>
							</div>
						</div>
		 			...
		 			if(resp.length > 0){
		 					Object.keys(resp).map(k=>{
		 						var obj = {};
		 							...
		 							obj['start']=resp[k].month_from+'T'+resp[k].time_from;
		 							obj['end']=resp[k].month_to+'T'+resp[k].time_to;
									 }
				          ...
				          eventClick: function(e,el) {
							   var data =  e.event.extendedProps;
								uni_modal('View Schedule Details','view_details.php?id='+data.data_id,'mid-large')
